...
public class BtVFTaskSwitcher: MonoBehaviour
{
    ...
    private GraspVfAgent graspVfAgent;
    ...
    void Start()
    {
        ...
        graspVfAgent = Grasp.GetComponent<GraspVfAgent>();
    ...
    {
        string directoryPath = "Assets/VFLogs";
        ...
        while (!fileCreated)
        {
            logFilePath = Path.Combine(directoryPath, $"VF_log_env_{fileIndex}.csv");
    ...
    void fSwitchToInsertion(Vector3 BTOffset, float [] Init_Angles)
    {
        if (NextFrameInt == 0)
        {
            ...
            graspVfAgent.ResetAllAB();
            graspVfAgent.Resetter();
            graspVfAgent.EndEpisode();
            graspVfAgent.enabled = false;
        }
    ...
    void FixedUpdate()
    {   
        if (graspVfAgent.HasSuccessfullyGrasped())
        {
        ...
        if (graspVfAgent.HasFailedGrasp())
        {
