...
public class PlatformAgent : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        // Set target to joints ignore link 6 here
        for (int i = 0; i < response.Angles.Count - 1; i++)
        {   
            //Debug.Log("Setting joint " + i + " to " + response.Angles[i]);
            links[i].SetDriveTarget(ArticulationDriveAxis.X, response.Angles[i]);
        }
        // Add another action to control the joint 6, roration of the gripper
        ...
        if (distanceToTarget > prevBest)
        {
            ...
            DistanceReward = DistanceReward + Dist_reward/ 1000.0f;
        }
        else
        {
            // Reward if the arm moves closer to target
            ...
            DistanceReward = DistanceReward + Dist_reward2/ 1000.0f;
        ...
        AngleReward = AngleReward - Angle_reward / 1000.0f;
    ...
    float CalculatePenalty(float Gripper_angle, float rotation_angle, float deviation)
    {
        ...
        return penalty + (penalty2 *1.0f) - 2.0f;
    }
