...
public class AgentInsertion : Agent
{
    ...
    {   
        if (requestCount!=0)
        {   
            float groundhitpen = -1.0f + requestCount * 0.1f;
            if (groundhitpen  > -0.5f)
            {
                groundhitpen = -0.5f;
            }
    ...
    {
        if (CollidedObject.name == "FingerA" || CollidedObject.name == "FingerB")
        {
        }
        else if (CollidedObject.name == "Cube" || CollidedObject.name == "BoxWithHole" )
        {
